"""
Scope Validation and Authorization Service

This service ensures all testing remains within authorized scope boundaries.
Core principle: Never test what you're not authorized to test.
"""
...
class ScopeValidator:
    ...
    def add_authorized_domain(self, domain: str):
        """Add domain to authorized scope"""
    ...
    def add_authorized_pattern(self, pattern: str):
        """Add regex pattern to authorized scope"""
    ...
    def is_in_scope(self, url: str) -> Tuple[bool, str, List[str]]:
        ...
        try:
            ...
            # If no authorized domains set, warn but allow (testing mode)
            ...
            if domain in self.authorized_domains:
                return True, "Domain is in authorized scope", warnings
            
            # Check subdomain match
            for auth_domain in self.authorized_domains:
                if domain.endswith(f".{auth_domain}"):
                    return True, f"Subdomain of authorized domain: {auth_domain}", warnings
            
            # Check pattern match
            for pattern in self.authorized_patterns:
                if re.match(pattern, url):
                    return True, f"Matches authorized pattern: {pattern}", warnings
            
            return False, "Target is not in authorized scope", warnings
    ...
    def validate_authorization_doc(self, doc: str) -> Tuple[bool, str]:
        """
        Validate authorization documentation
        ...
        if not doc or len(doc.strip()) < 50:
            return False, "Authorization documentation is too brief"
        
        required_keywords = ["scope", "authorize", "permission"]
        doc_lower = doc.lower()
        
        found_keywords = [kw for kw in required_keywords if kw in doc_lower]
        
        if len(found_keywords) < 2:
            return False, "Authorization documentation missing key information"
        
        return True, "Authorization documentation appears valid"
